...
import { patch  } from 'core/platform/web/patch'
import * as nodeOps from 'core/platform/web/node-ops'
...
describe('vdom patch: hooks', () => {
  ...
  // 失败问题在，这个vnode没有parentnode，这个vnode是root，所以需要特殊处理，要去看一下vue源码里具体是如何处理的。
  it('should invoke the remove hook on replaced root', () => {
    ...
    let re = patch(vnode0, vnode1)
    re = patch(vnode1, vnode2)
    expect(result.length).toBe(1)
  })
